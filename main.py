        ...
        def timer():
            ...
            try:
                ...
                while len(doneusers) > 0:
                    ...
                    with busyLock:
                        cur = db.cursor()
                        cur.execute("SELECT name FROM users WHERE name IN(%s)" % ",".join(["%s"] * len(currentSlice)),
                                    currentSlice)
    ...
    def do_command(self, command, args, sender, channel, tags, isWhisper=False):
        ...
        if command == "as" and debugMode and sender in superadmins:
            ...
            self.do_command(args[1][1:].lower(), args[2:], args[0].lower(), channel,
                            {'display-name': args[0], 'user-id': userid}, isWhisper)
            return
        with busyLock:
            ...
            if command == "disenchant" or command == "de":
                ...
                if len(disenchants) == 1:
                    ...
                    self.message(channel,
                                 ...
                                 len(disenchants), buytext, pointsGain, str(tags['display-name'])), isWhisper=isWhisper)
                ...
                with db.cursor() as cur:
                    ...
                    if points + currAmount >= required:
                        ...
                        self.message(channel,
                                     ...
                                     tags['display-name'], points, title, required - currAmount - points), isWhisper)
                        ...
                        if betRow is None or (betRow[1] != 'paid' and betRow[1] != 'completed'):
                            ...
                            for winner in resultData["winners"]:
                                ...
                                if numEntries > int(config["betParticipantScalingCap"]):
                                    prize *= (100 * int(config["betParticipantScalingCap"]) + 60) / (
                                                100 * numEntries + 60)
                            ...
                            bcPrize = min(max(resultData["result"] / 7200.0, max(prizes) / 2.0, 50), max(prizes),
                                          resultData["result"] / 2400.0)
                            ...
                            cur.execute(
                                "UPDATE bets SET status = 'paid', totalPaid = %s, paidBroadcaster = %s, paidAt = %s WHERE id = %s",
                                [paidOut, bcPrize, current_milli_time(), betRow[0]])

                            # take away points from the bot account
                            cur.execute("UPDATE users SET points = points - %s WHERE name = %s",
                                        [paidOut, config["username"]])
                    ...
                    try:
                        ...
                        if outbidding:
                            ...
                            self.message(channel,
                                         ...
                                         tags['display-name'], highest_other_bid + minimum_outbid, highest_other_bid))
                            cur.close()
                            return
                        ...
                        if outbidding:
                            ...
                            cur.execute(
                                "SELECT users.name FROM bounties JOIN users ON bounties.userid=users.id WHERE bounties.waifuid = %s AND bounties.amount = %s LIMIT 1",
            ...
            if command == "profile":
                if len(args) == 0:
                    ...
                    if not canLookup and not hasOrIsLowRarity:
                        ...
                        self.message(channel, "Updated your favourite Waifu to be " + newFavW["name"] + "! naroDesu", isWhisper)
                    ...
                    if len(newDesc) > 1023:
                        self.message(channel, "That description is too long. Please limit it to 1024 characters.", isWhisper)
                        return
