...
def startBet(channel, confirmed=False):
    with db.cursor() as cur:
        ...
        if row is not None:
            ...
            cur.execute("UPDATE bets SET startTime = %s, status = 'started' WHERE id = %s",
                        [current_milli_time(), row[0]])
        else:
...
def openBet(channel):
    ...
    if result > 0:
        ...
        cur.execute("INSERT INTO bets(channel, status, openedTime) VALUES (%s, 'open', %s)",
                    [channel, current_milli_time()])
        cur.close()
        return True
...
def getHand(twitchid):
    ...
    with db.cursor(pymysql.cursors.DictCursor) as cur:
        cur.execute(
            "SELECT cards.id AS cardid, waifus.name, waifus.id AS waifuid, cards.rarity, waifus.series, COALESCE(cards.customImage, waifus.image) AS image, waifus.base_rarity, cards.tradeableAt FROM cards JOIN waifus ON cards.waifuid = waifus.id WHERE cards.userid = %s AND cards.boosterid IS NULL ORDER BY COALESCE(cards.sortValue, 32000) ASC, (rarity < %s) DESC, waifus.id ASC, cards.id ASC",
...
def getOpenBooster(twitchid):
    try:
        ...
        return []
    with db.cursor(pymysql.cursors.DictCursor) as cur:
        cur.execute("SELECT id, boostername, paid FROM boosters_opened WHERE userid = %s AND status = 'open' LIMIT 1",
                    [tID])
        booster = cur.fetchone()
        if booster is None:
            return None
        cur.execute(
            "SELECT cards.id AS cardid, waifus.name, waifus.id AS waifuid, cards.rarity, waifus.series, COALESCE(cards.customImage, waifus.image) AS image, waifus.base_rarity FROM cards JOIN waifus ON cards.waifuid = waifus.id WHERE cards.userid = %s AND cards.boosterid = %s ORDER BY waifus.id ASC",
            [tID, booster['id']])
...
def getCard(cardID):
    with db.cursor(pymysql.cursors.DictCursor) as cur:
        cur.execute(
            "SELECT cards.*, waifus.base_rarity FROM cards JOIN waifus ON cards.waifuid=waifus.id WHERE cards.id = %s",
            [cardID])
        return cur.fetchone()
...
def addCard(userid, waifuid, source, boosterid=None, rarity=None):
    with db.cursor() as cur:
        ...
        cur.execute(
            "INSERT INTO cards (userid, boosterid, waifuid, rarity, created, originalOwner, originalBooster, source) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)",
            waifuInfo)
...
def updateCard(id, changes):
    ...
    with db.cursor() as cur:
        cur.execute('UPDATE cards SET {} WHERE id = %s'.format(', '.join('{}=%s'.format(k) for k in changes)),
                    list(changes.values()) + [id])
...
def disenchant(bot, cardid):
    ...
    with db.cursor() as cur:
        ...
        if order is not None:
            ...
            deValue += (min_bounty - base_value) + max(min(order[3] - min_bounty, rarity_cap - min_bounty) // 2,
                                                       0) + max((order[3] - rarity_cap) // 4, 0)

        return deValue


def sendPushNotification(ids, data):
    pushHeaders = {'Authorization': 'Basic ' + base64.encode('internal:' + config['adminPass'])}
...
def checkFavouriteValidity(userid):
    with db.cursor() as cur:
        ...
        if favourite["can_favourite"] == 0:
            ...
            # must be owned
            cur.execute("SELECT COUNT(*) FROM cards WHERE waifuid = %s AND boosterid IS NULL AND userid = %s",
                        [favourite["id"], userid])
...
def updateBoth(game, title):
    ...
    r = requests.put("https://api.twitch.tv/kraken/channels/" + config["marathonChannelID"], headers=myheaders,
                     json=body)
    try:
        j = r.json()
...
def sendSetAlert(channel, user, name, waifus, reward, firstTime, discord=True):
    ...
    discordbody = {"username": "Waifu TCG", "embeds": [
        ...
        },
        {
            ...
            "description": "They gathered {waifus} and received {reward} as their reward.".format(
                waifus=naturalJoinNames(waifus), reward=reward),
            "url": "https://twitch.tv/{name}".format(name=str(channel).replace("#", "").lower()),
            "color": int(config["rarity" + str(int(config["numNormalRarities"]) - 1) + "EmbedColor"]),
            "footer": {
                "text": "Waifu TCG by Marenthyu"
...
def getWaifuById(id):
    ...
    with db.cursor(pymysql.cursors.DictCursor) as cur:
        cur.execute(
            "SELECT id, name, image, base_rarity, series, can_lookup, pulls, last_pull, can_favourite, can_purchase FROM waifus WHERE id=%s",
            [id])
        return cur.fetchone()
    ...
    for i in range(2):
        ...
        for owner in ownerData:
            if len(ownerData[owner]) != 1 or baseRarityName not in ownerData[owner] or ownedByOwner[owner] > 1:
                # verbose
                ownerDescriptions[i].append(owner + " (" + ", ".join(
                    [("%d %s" % (ownerData[owner][k], k) if ownerData[owner][k] > 1 else k) for k in
                     ownerData[owner]]) + ")")
            else:
...
def takePudding(userid, amount):
    ...
    with db.cursor() as cur:
        cur.execute("UPDATE users SET puddingCurrent = %s, puddingPrevious = %s, puddingExpiring = %s WHERE id = %s",
                    pinfo + [userid])
...
def getUniqueCards(userid):
    with db.cursor() as cur:
        ...
        if len(uniqueRarities) == 0:
            ...
            cur.execute(
                "SELECT DISTINCT waifuid FROM cards WHERE userid = %s AND boosterid IS NULL AND rarity IN ({0})".format(
                    inStr),
    ...
    if rarity == -1:
        ...
        with db.cursor() as cur:
            ...
            if rarity >= int(config["strongerWeightingMinRarity"]):
                cur.execute(
                    "SELECT id FROM waifus WHERE base_rarity = %s{1} AND normal_weighting >= 1 ORDER BY -LOG(1-RAND())/{0} LIMIT 1".format(
                ...
                for row in candidates:
                    ...
                    if rarity < int(config["numNormalRarities"]) - 1 and amount >= int(
                        ...
                        if amountToMake != 0:
                            ...
                            cur.execute(
                                "SELECT id, COALESCE(tradeableAt, 0) FROM cards WHERE waifuid = %s AND userid = %s AND rarity = %s AND boosterid IS NULL ORDER BY tradeableAt ASC",
                                [waifuid, userid, rarity])
                            fodder = cur.fetchall()
...
def openBooster(bot, userid, username, display_name, channel, isWhisper, packname, buying=True, mega=False):
    with db.cursor() as cur:
        ...
        for iter in range(iterations):
            ...
            if packname in packAmountRewards and iterDE in packAmountRewards[packname]:
                ...
                bot.message("#%s" % username,
                            "You won a free %s pack due to getting a %s pack worth %d points. Open it with !freepacks open %s" % msgArgs,
                            True)
        ...
        boosterid = addBooster(userid, logPackName, cost * iterations if buying else 0, 'open', totalTokensDropped,
                               channel, isWhisper)
        for card in cards:
    ...
    while not hasSeed:
        ...
        for i in range(int(numGoodRewards)):
            ...
            while order[pos] >= numGoodRewards:
                pos += 1
            if pos - lastPos <= (cycleLength / numGoodRewards) / 2 or pos - lastPos >= (
                    cycleLength / numGoodRewards) * 2:
                hasSeed = False
                break
            lastPos = pos
...
class NepBot(NepBotClass):
            ...
            with busyLock:
                ...
                with db.cursor() as cur:
                    ...
                    for pack in packsToClose:
                        ...
                        nonSpaceCards = sum(
                            (1 if card['base_rarity'] >= int(config["numNormalRarities"]) else 0) for card in cards)
                    ...
                    if int(config["last_weighting_update"]) < current_milli_time() - int(
                        ...
                        cur.execute(
                            "UPDATE waifus SET normal_weighting = normal_weighting * %s WHERE base_rarity BETWEEN %s AND %s",
                            [baseIncrease ** 2, int(config["strongerWeightingMinRarity"]),
                             int(config["numNormalRarities"]) - 1])
                    ...
                    if ymdNow > config["last_pudding_check"]:
                        ...
                        if now.day == 1:
                            ...
                            cur.execute(
                                "UPDATE users SET puddingExpiring = puddingPrevious, puddingPrevious = puddingCurrent, puddingCurrent = 0")
                            cur.execute("SELECT name, puddingExpiring FROM users WHERE puddingExpiring > 0")
                            for userRow in cur.fetchall():
                                self.message("#%s" % userRow[0],
                                             "You have %d pudding expiring on the 8th of this month. !pudding to see your balance and to spend it." %
                                             userRow[1], True)
                        elif now.day == 8:
                    ...
                    if booleanConfig("cpuWarActive") and now.minute < 5:
                        cpuVoters.clear()
                        self.message(config["marathonChannel"], "**You can now cast your votes for a CPU again!**",
                                     False)
    ...
    def on_message(self, source, target, message, tags, isWhisper=False):
        ...
        activeCommands = ["checkhand", "points", "pudding", "freewaifu", "freebie", "disenchant",
                          ...
                          "sets", "set", "setbadge", "giveaway", "raffle", "bounty", "profile", "packspending",
                          "godimage",
                          "sendpoints", "sorthand"]
        ...
        if userid in blacklist:
            if message.startswith("!") and message.split()[0][1:].lower() in activeCommands:
                self.message(source,
                             "Account banned from playing TCG. If you're seeing this without knowing why, contact TCG staff @ %s/discord" %
                             config["siteHost"], isWhisper)
            return
        elif "bot" in sender:
            with busyLock:
                with db.cursor() as cur:
                    ...
                    if banrow is None:
                        ...
                        if message.startswith("!") and message.split()[0][1:].lower() in activeCommands:
                            self.message(source,
                                         "This account appears to be a bot. If it is not, contact TCG staff @ %s/discord to have it unbanned." %
                                         config["siteHost"], isWhisper)
                        return
                    if banrow[1] > 0:
    ...
    def do_command(self, command, args, sender, channel, tags, isWhisper=False):
        ...
        with busyLock:
            ...
            if command == "cpu" and isMarathonChannel and booleanConfig("cpuWarActive"):
                ...
                if cpu in ["neptune", "noire", "blanc", "vert"]:
                    ...
                    warStrings = ["%s - %d votes" % ("HDN" + row[0][0].upper() + row[0][1:], row[1]) for row in
                                  currentVotes]
            ...
            if command == "points":
                with db.cursor() as cur:
                    ...
                    self.message(channel,
                                 "%s, you have %d points and %d pudding!" % (tags['display-name'], points, pudding),
                                 isWhisper)
                    return
            if command == "pudding":
                ...
                if subcmd == "booster" or subcmd == "buy":
                    ...
                    if boostername.startswith("mega"):
                        ...
                        mega = True
                    with db.cursor() as cur:
                        cur.execute("SELECT name, cost, canMega FROM boosters WHERE name = %s AND buyable = 1",
                                    [truename])
                        booster = cur.fetchone()
                        ...
                        if not hasPudding(tags['user-id'], cost):
                            self.message(channel, "%s, you can't afford a %s booster. They cost %d pudding." % (
                                tags['display-name'], boostername, cost), isWhisper)
                            return
                        takePudding(tags['user-id'], cost)
                        try:
                            openBooster(self, tags['user-id'], sender, tags['display-name'], channel, isWhisper,
                                        truename, False, mega)
                            if checkHandUpgrade(tags['user-id']):
                                messageForHandUpgrade(tags['user-id'], tags['display-name'], self, channel, isWhisper)
                            self.message(channel, "%s, you open a %s booster for %d pudding: %s/booster?user=%s" % (
                                tags['display-name'], boostername, cost, config["siteHost"], sender), isWhisper)
                    ...
                    with db.cursor() as cur:
                        cur.execute(
                            "SELECT name, cost FROM boosters WHERE listed = 1 AND buyable = 1 ORDER BY sortIndex ASC")
                        boosters = cur.fetchall()
                        boosterInfo = ", ".join(
                            "%s / %d pudding" % (row[0], math.ceil(int(row[1]) / int(config["puddingExchangeRate"])))
                            for row in boosters)
                        self.message(channel,
                                     "Current buyable packs: %s. !pudding booster <name> to buy a booster with pudding." % boosterInfo,
                                     isWhisper)
                elif subcmd == "topup":
                    if len(args) < 2:
                    ...
                    if not hasPoints(tags['user-id'], pointsCost):
                        self.message(channel, "%s, you don't have enough points to buy %d pudding. You need %d." % (
                            tags['display-name'], amount, pointsCost), isWhisper)
                        return
                    ...
                    self.message(channel,
                                 "%s, you spent %d points to buy %d extra pudding. You now have %d total pudding (!pudding for full details)" % msgArgs,
                                 isWhisper)
                else:
                    ...
                    if sum(pudding) == 0:
                        self.message(channel,
                                     "%s, you don't currently have any pudding. You can earn some by participating in bets or completing sets." %
                                     tags['display-name'], isWhisper)
                    else:
                        msgArgs = (tags['display-name'], sum(pudding), pudding[0], pudding[1], pudding[2],
                                   config["puddingExchangeRate"])
                        self.message(channel,
                                     "%s, you have %d total pudding: %d earned this month, %d earned last month, %d expiring soon. !pudding list to see what boosters you can buy, !pudding booster <name> to buy a booster with pudding. !pudding topup <amount> to buy extra pudding (%s points each)" % msgArgs,
                                     isWhisper)
                return
            if command == "freewaifu" or command == "freebie":
                # print("Checking free waifu egliability for " + str(sender))
                with db.cursor() as cur:
                    cur.execute("SELECT lastFree, rewardSeqSeed, rewardSeqIndex FROM users WHERE id = %s",
                                [tags['user-id']])
                    res = cur.fetchone()
                    ...
                    cur.execute(
                        "SELECT points, waifuid, waifu_rarity, boostername FROM free_rewards WHERE `is_good` = %s AND `index` = %s",
                        lookup)
                    ...
                    if (rewardInfo[1] is not None or rewardInfo[2] is not None) and hasPack and not spaceInHand:
                        ...
                        cur.execute("UPDATE users SET rewardSeqSeed = %s, rewardSeqIndex = %s WHERE id = %s",
                                    [seed, index, tags['user-id']])
                        self.message(channel,
                                     "%s, your hand is full and you have a booster open!" % tags['display-name'],
                                     isWhisper)
                        return
                    ...
                    if rewardInfo[0] is not None:
                        ...
                        if cardRewardCount == 0:
                            self.message(channel, "%s, you got your daily free reward: %d points!" % (
                                tags['display-name'], rewardInfo[0]), isWhisper)
                    ...
                    if rewardInfo[1] is not None or rewardInfo[2] is not None:
                        ...
                        if row['base_rarity'] >= int(config["drawAlertMinimumRarity"]):
                            threading.Thread(target=sendDrawAlert,
                                             args=(channel, row, str(tags["display-name"]))).start()
                        ...
                        self.message(channel,
                                     "{username}, you got your daily free reward: {points}[{id}][{rarity}] {name} from {series} - {link}{pack}".format(
                                         **msgArgs), isWhisper)
                    ...
                    if rewardInfo[3] is not None:
                        if hasPack:
                            ...
                            self.message(channel,
                                         "%s, you got your daily free reward: %sa %s booster (sent to !freepacks)" % (
                                             tags['display-name'], pointsPrefix, rewardInfo[3]), isWhisper)
                        else:
                            try:
                                packid = openBooster(self, tags['user-id'], sender, tags['display-name'], channel,
                                                     isWhisper, rewardInfo[3], False)
                                if checkHandUpgrade(tags['user-id']):
                                    messageForHandUpgrade(tags['user-id'], tags['display-name'], self, channel,
                                                          isWhisper)
                                self.message(channel,
                                             "%s, you got your daily free reward: %sa %s booster - %s/booster?user=%s" % (
                                                 tags['display-name'], pointsPrefix, rewardInfo[3], config['siteHost'],
                                                 sender), isWhisper)
                            except InvalidBoosterException:
                                discordbody = {
                                    "username": "WTCG Admin",
                                ...
                                threading.Thread(target=sendAdminDiscordAlert, args=(discordbody,)).start()
                                self.message(channel, "Could not retrieve your free reward, try again later.",
                                             isWhisper)
                                return

                    cur.execute("UPDATE users SET lastFree = %s, rewardSeqSeed = %s, rewardSeqIndex = %s WHERE id = %s",
                                [current_milli_time(), seed, index + 1, tags['user-id']])
                    return
            if command == "disenchant" or command == "de":
                ...
                for row in disenchants:
                    ...
                    if row['rarity'] == godRarity:
                        ...
                        with db.cursor() as cur:
                            cur.execute(
                                "UPDATE godimage_requests SET state='cancelled' WHERE requesterid = %s AND cardid = %s AND state = 'pending'",
                                [tags['user-id'], row['cardid']])
                            if cur.rowcount > 0:
                                # request was cancelled
                                waifuData = getWaifuById(row['waifuid'])
                                self.message("#%s" % sender,
                                             "Your image change request for [%d] %s was cancelled since you disenchanted it." % (
                                                 row['waifuid'], waifuData['name']), True)
            ...
            if command == "booster":
                ...
                if cmd == "select":
                    ...
                    for card in deCards:
                        ...
                        if gain > baseValue:
                            ...
                            threading.Thread(target=sendDisenchantAlert,
                                             args=(
                                                 channel, getWaifuById(card['waifuid']),
                ...
                if cmd == "list":
                    with db.cursor() as cur:
                        ...
                        self.message(channel,
                                     "Current buyable packs: %s. !booster buy <name> to buy a booster with points." % boosterInfo,
                                     isWhisper)
                    return
            ...
            if command == "trade":
                ...
                with db.cursor() as cur:
                    ...
                    if subarg == "list":
                        cur.execute(
                            "SELECT users.name FROM trades JOIN users ON trades.toid = users.id WHERE trades.fromid = %s AND trades.status = 'open'",
                            [ourid])
                        sentTradesTo = [row[0] for row in cur.fetchall()]

                        cur.execute(
                            "SELECT users.name FROM trades JOIN users ON trades.fromid = users.id WHERE trades.toid = %s AND trades.status = 'open'",
                            [ourid])
                        ...
                        if len(sentTradesTo) == 0 and len(recvdTradesFrom) == 0:
                            ...
                            self.message(channel, "%s, you have %d pending trade%s. %s" % (
                                tags['display-name'], numTrades, "s" if numTrades > 1 else "", " ".join(parts)),
                                         isWhisper)
                        return
                    if subarg in ["check", "accept", "decline", "cancel"]:
                        ...
                        if subarg == "cancel":
                            ...
                            cur.execute(
                                "SELECT id FROM trades WHERE fromid = %s AND toid = %s AND status = 'open' LIMIT 1",
                                [ourid, otherid])
                            trade = cur.fetchone()

                            if trade is None:
                                self.message(channel,
                                             "You do not have a pending trade with %s. Send one with !trade %s <have> <want>" % (
                                                 otherparty, otherparty), isWhisper)
                            else:
                                cur.execute("UPDATE trades SET status = 'cancelled', updated = %s WHERE id = %s",
                                            [current_milli_time(), trade[0]])
                                self.message(channel, "You cancelled your pending trade with %s." % otherparty,
                                             isWhisper)
                        ...
                        if want['userid'] is None or want['userid'] != ourid or have['userid'] is None or have[
                            'userid'] != otherid:
                            self.message(channel,
                                         "%s, one or more of the cards involved in the trade are no longer in their owners hands. Trade cancelled." %
                                         tags['display-name'], isWhisper)
                            cur.execute("UPDATE trades SET status = 'invalid', updated = %s WHERE id = %s",
                                        [current_milli_time(), trade[0]])
                            return

                        if (want['tradeableAt'] is not None and want['tradeableAt'] > current_milli_time()) or (
                                have['tradeableAt'] is not None and have['tradeableAt'] > current_milli_time()):
                            self.message(channel,
                                         "%s, one or more of the cards involved in the trade are no longer able to be traded. Trade cancelled." %
                                         tags['display-name'], isWhisper)
                            cur.execute("UPDATE trades SET status = 'invalid', updated = %s WHERE id = %s",
                                        [current_milli_time(), trade[0]])
                            return
                        ...
                        if subarg == "check":
                            ...
                            haveStr = getWaifuRepresentationString(havedata['id'], havedata['base_rarity'],
                                                                   have['rarity'], havedata['name'])
                            wantStr = getWaifuRepresentationString(wantdata['id'], wantdata['base_rarity'],
                                                                   want['rarity'], wantdata['name'])
                            ...
                            if want['rarity'] == godRarity:
                                cur.execute(
                                    "UPDATE godimage_requests SET state = 'cancelled' WHERE cardid = %s AND state = 'pending'",
                                    [want['id']])
                                if cur.rowcount > 0:
                                    # a request was actually cancelled
                                    wantdata = getWaifuById(want['waifuid'])
                                    self.message("#%s" % sender,
                                                 "Your image change request for [%d] %s was cancelled since you traded it away." % (
                                                     want['waifuid'], wantdata['name']), True)
                            ...
                            if have['rarity'] == godRarity:
                                ...
                                if cur.rowcount > 0:
                                    # a request was actually cancelled
                                    havedata = getWaifuById(have['waifuid'])
                                    self.message("#%s" % otherparty,
                                                 "Your image change request for [%d] %s was cancelled since you traded it away." % (
                                                     have['waifuid'], havedata['name']), True)
                            ...
                            # I am not sure if it is otherid or ourid here.
                ...
                if have["tradeableAt"] is not None and have["tradeableAt"] > current_milli_time():
                    self.message(channel,
                                 "%s, the card you are attempting to send in the trade is not tradeable right now!" %
                                 tags['display-name'], isWhisper)
                    return
                ...
                if want["tradeableAt"] is not None and want["tradeableAt"] > current_milli_time():
                    self.message(channel,
                                 "%s, the card you are attempting to receive in the trade is not tradeable right now!" %
                                 tags['display-name'], isWhisper)
                    return
        ...
        if command == "togglehoraro" and sender in admins and booleanConfig("marathonBotFunctions"):
            ...
            return
        if sender in admins and command in ["status", "title"] and isMarathonChannel and booleanConfig(
                "marathonBotFunctions"):
        ...
        if sender in admins and booleanConfig("marathonBotFunctions") and command == "ffzfollowing":
            ...
            self.message(channel, "%s -> Attempted to update follower buttons to %s." % (
                tags['display-name'], ", ".join(args)))
            return
        if command == "emotewar":
        ...
        if command == "redeem":
            ...
            if redeemdata[3] is not None:
                ...
                try:
                    packid = openBooster(self, tags['user-id'], sender, tags['display-name'], channel, isWhisper,
                                         redeemdata[3],
                                         False)
                    if checkHandUpgrade(tags['user-id']):
                        messageForHandUpgrade(tags['user-id'], tags['display-name'], self, channel, isWhisper)
            ...
            with db.cursor() as cur:
                ...
                if war is not None:
                    ...
                    try:
                        ...
                        self.message(channel,
                                     "%s -> Invalid amount of points/pudding entered." % tags['display-name'])
                        return
                    ...
                    if theirchoiceL not in choiceslookup:
                        ...
                        if contribution < openEntryMinimum:
                            self.message(channel,
                                         "%s -> You must contribute at least %d points or %d pudding to add a new choice to this bidwar!" % (
                                             tags['display-name'], openEntryMinimum,
                                             math.ceil(openEntryMinimum / exchangeRate)), isWhisper)
                            return
                        ...
                        qargs = [warid, theirchoice, contribution, actionTime, tags['user-id'], actionTime,
                                 tags['user-id']]
                        cur.execute(
                            "INSERT INTO bidWarChoices (warID, choice, amount, created, creator, lastVote, lastVoter) VALUES(%s, %s, %s, %s, %s, %s, %s)",
                            qargs)

                        logargs = [tags['user-id'], warid, theirchoice, points, contribution, currency,
                                   current_milli_time()]
                    else:
                        ...
                        cur.execute(
                            ...
                            qargs)
                        logargs = [tags['user-id'], warid, theirchoiceL, points, contribution, currency,
                                   current_milli_time()]

                    cur.execute(
                        "INSERT INTO `contributionLog` (`userid`, `to_id`, `to_choice`, `raw_amount`, `contribution`, `currency`, `timestamp`) " +
                    ...
                    cur.execute(
                        "SELECT id, title, amount, required FROM incentives WHERE id = %s AND status = 'open'",
                        [args[0]])
                    ...
                    if contribution + currAmount >= required:
                        ...
                        self.message(channel,
                                     "%s -> You successfully donated %s towards the %s incentive. It needs %d more points to be met." % (
                                         tags['display-name'], contributionStr, title,
                                         (required - currAmount - contribution) * memeMult),
                                     isWhisper)
            ...
            with db.cursor() as cur:
                ...
                for ic in cur.fetchall():
                    ...
                    if ic[2] >= ic[3]:
                        ...
                        incentives.append(
                            "%s%s (%s) - %d/%d points" % (
                                "; " if incnum > 1 else "", ic[1], ic[0], ic[2] * 10000000, ic[3] * 10000000))
                    else:
                        incentives.append(
        ...
        if command == "recheckpromos" and sender in superadmins:
            with db.cursor() as cur:
                cur.execute(
                    "SELECT DISTINCT waifuid FROM cards WHERE userid IS NOT NULL AND boosterid IS NULL GROUP BY userid, waifuid, rarity HAVING COUNT(*) >= 2")
                rows = cur.fetchall()
                ids = [row[0] for row in rows]
                attemptPromotions(*ids)
                self.message(channel, "Rechecked promotions for %d waifus" % len(ids))
            return
        if command == "freepacks" or command == "freepack" or (
                command == "bet" and len(args) > 0 and args[0].lower() == "packs"):
            if len(args) > 0 and args[0].lower() in ["open", "claim", "redeem"]:
                ...
                with db.cursor() as cur:
                    cur.execute(
                        "SELECT remaining, boostername FROM freepacks WHERE userid = %s AND boostername = %s",
                        [tags['user-id'], args[1]])
                    result = cur.fetchone()

                    if result is None or result[0] == 0:
                        self.message(channel, "You don't have any free packs of that type left to claim!",
                                     isWhisper)
                        return
                    ...
                    try:
                        packid = openBooster(self, tags['user-id'], sender, tags['display-name'], channel,
                                             isWhisper, args[1], False)
                        if checkHandUpgrade(tags['user-id']):
                            messageForHandUpgrade(tags['user-id'], tags['display-name'], self, channel, isWhisper)
                        cur.execute(
                            "UPDATE freepacks SET remaining = remaining - 1 WHERE userid = %s AND boostername = %s",
                            [tags['user-id'], args[1]])
                        self.message(channel, "%s, you open a free %s booster: %s/booster?user=%s" % (
                            tags['display-name'], result[1], config["siteHost"], sender), isWhisper)
                    except InvalidBoosterException:
                        discordbody = {
                            "username": "WTCG Admin",
                ...
                with db.cursor() as cur:
                    cur.execute("SELECT boostername, remaining FROM freepacks WHERE userid = %s AND remaining > 0",
                                [tags['user-id']])
                    freepacks = cur.fetchall()

                    if len(freepacks) == 0:
                        self.message(channel, "%s, you don't have any free pack entitlements right now." % tags[
                            'display-name'], isWhisper)
                    else:
                        freeStr = ", ".join("%s x%d" % (fp[0], fp[1]) for fp in freepacks)
                        self.message(channel,
                                     "%s, your current free packs: %s. !freepacks open <name> to open one." % (
                                         tags['display-name'], freeStr), isWhisper)
                    return
        if command == "bet":
            if isWhisper:
            ...
            if bet:
                ...
                if canManageBets and subcmd == "open":
                    ...
                    try:
                        ...
                        self.message(channel,
                                     "WARNING: This bet does not currently have enough participants to be eligible for payout. To start anyway, use !bet start yes")
                    except NotOpenLongEnoughException:
                        self.message(channel,
                                     "You must wait at least %d minutes after opening a bet to start it." % int(
                                         config["betMinimumMinutesOpen"]))
                    ...
                    if resultData is None:
                        ...
                        if not canAdminBets and len(winners) >= int(config["betMinimumEntriesForPayout"]):
                            ...
                            discordArgs = {"channel": chanStr, "time": formattedTime,
                                           "link": "https://twitch.tv/" + chanStr}
                            discordbody = {
                                "username": "WTCG Admin",
                                "content": "A bet has just finished in {channel} with a time of {time}. Check results and consider payout at <{link}>.".format(
                                    **discordArgs)
                            }
                            threading.Thread(target=sendAdminDiscordAlert, args=(discordbody,)).start()
                    return
                    ...
                    cur = db.cursor()
                    cur.execute(
                        "SELECT id, status FROM bets WHERE channel = %s AND `status` != 'open' ORDER BY id DESC LIMIT 1",
                        [channel])
                    betRow = cur.fetchone()
                    if betRow is None:
                        self.message(channel, "No time prediction contests have been done in this channel yet.",
                    ...
                    with db.cursor() as cur:
                        ...
                        if betRow is None or betRow[1] != 'started':
                            ...
                            if '#' + sender == channel:
                                ...
                                if restrictions[1] != 0:
                                    self.message(channel,
                                                 "This channel is banned from using self forcereset at the present time.")
                                    return
                                invocation = current_milli_time()
                                period = invocation - int(config["betForceResetPeriod"])
                                cur.execute(
                                    "SELECT COUNT(*), MIN(`timestamp`) FROM forceresets WHERE channel = %s AND `timestamp` > %s",
                                    [channel, period])
                                frData = cur.fetchone()
                                if frData[0] >= int(config["betForceResetLimit"]):
                                    ...
                                    self.message(channel,
                                                 "You are currently out of self forceresets. Your next one will be available in %s." % datestring)
                                    return
                                cur.execute(
                                    "INSERT INTO forceresets (channel, user, `timestamp`) VALUES(%s, %s, %s)",
                                    [channel, tags['user-id'], invocation])
                    ...
                    if betRow is None or (betRow[1] != 'paid' and betRow[1] != 'completed'):
                        ...
                        if numEntries < int(config["betMinimumEntriesForPayout"]):
                            self.message(channel, "This contest had less than %d entrants, no payout." % int(
                                config["betMinimumEntriesForPayout"]), isWhisper)
                            cur.close()
                            return
                        ...
                        for winner in resultData["winners"]:
                            ...
                            if abs(winner["timedelta"]) < 1000 and canWinBigPrizes:
                                ...
                                msg = "You won a %s booster from the bet in %s's channel. Open it in any chat with !freepacks open %s" % (
                                    booster, channel[1:], booster)
                                prizeStrings.append("%s - %s pack" % (winner["name"], booster))
                                cur.execute(
                                    "UPDATE placed_bets SET prizePack = %s WHERE betid = %s AND userid = %s",
                                    [booster, betRow[0], winner["id"]])
                            else:
                                pudding = minPrize + (maxPrize - minPrize) * (numEntries - place) / (
                                        numEntries - 1) / (1.4 if place > numEntries / 2 else 1)
                                if place == 1:
                                    pudding *= 1.3
                                ...
                                if canWinBigPrizes and abs(winner["timedelta"]) < resultData["result"] / 120:
                                    pudding *= 1.5
                                if winner["bet"] < resultData["result"] / 2 or winner["bet"] > resultData[
                                    "result"] * 2:
                                    pudding *= 0.5
                                pudding = round(pudding)
                                addPudding(winner["id"], pudding)
                                msg = "You won %d pudding from the bet in %s's channel. Check and spend it with !pudding" % (
                                    pudding, channel[1:])
                                prizeStrings.append("%s - %d pudding" % (winner["name"], pudding))
                                cur.execute(
                                    "UPDATE placed_bets SET prizePudding = %s WHERE betid = %s AND userid = %s",
                        ...
                        if not isMarathonChannel:
                            cur.execute(
                                "SELECT COALESCE(SUM(paidBroadcaster), 0) FROM bets WHERE status='paid' AND SUBSTRING(FROM_UNIXTIME(startTime/1000),1,7)=SUBSTRING(NOW(),1,7) AND channel = %s",
                                [channel])
                            ...
                            if puddingMonth + bcPrize > int(config["maxMonthlyBCReward"]):
                                ...
                                capped = True
                        prizeStrings.append("%s (broadcaster) - %d pudding%s" % (
                            channel[1:], bcPrize, " (monthly cap reached)" if capped else ""))
                        whispers.append((channel,
                                         "You were rewarded %d pudding%s for running your recent bet. Check and spend it with !pudding" % (
                                             bcPrize, " (monthly cap reached)" if capped else "")))
                        # skip using addPudding to save a database lookup
                        cur.execute("UPDATE users SET puddingCurrent = puddingCurrent + %s WHERE name = %s",
                                    [bcPrize, channel[1:]])
        ...
        if command == "sets" or command == "set":
            ...
            if subcmd == "claim":
                with db.cursor(pymysql.cursors.DictCursor) as cur:
                    ...
                    cur.execute(
                        "SELECT sets.* FROM sets LEFT JOIN sets_claimed ON (sets.id=sets_claimed.setid AND sets_claimed.userid = %s) WHERE sets.claimable = 1 AND sets_claimed.userid IS NULL AND id NOT IN(SELECT DISTINCT setID FROM set_cards LEFT JOIN cards ON (set_cards.cardID=cards.waifuid AND cards.userid=%s) WHERE cards.userid IS NULL)",
                        [tags['user-id']] * 2)

                    for row in cur.fetchall():
                        claimable += 1
                        if row["lastClaimTime"] is not None:
                            cooldown = row["lastClaimTime"] + int(
                                config["setCooldownDays"]) * 86400000 - current_milli_time()
                            if cooldown > 0:
                                # on cooldown
                                datestring = formatTimeDelta(cooldown, False)
                                self.message(channel,
                                             "Could not claim the set %s as it is on cooldown, try again in %s" % (
                                                 row["name"], datestring), isWhisper)
                                continue
                        ...
                        if row["firstClaimer"] is None:
                            ...
                            rPts *= 2
                            rPud *= 2
                            badgeid = addBadge(row["name"], config["setBadgeDescription"],
                        ...
                        cur.execute(
                            "INSERT INTO sets_claimed (setid, userid, rewardPoints, rewardPudding, timestamp) VALUES(%s, %s, %s, %s, %s)",
                            scValues)

                        firstClaimer = tags['user-id'] if first else row["firstClaimer"]
                        cur.execute(
                            "UPDATE sets SET firstClaimer = %s, lastClaimTime = %s, badgeid = %s WHERE id = %s",
                            [firstClaimer, claimTime, badgeid, row["id"]])
                        ...
                        self.message(channel,
                                     "Successfully claimed the set %s%s and rewarded %s with %s!" % msgArgs,
                                     isWhisper)
                        ...
                        threading.Thread(target=sendSetAlert,
                                         args=(
                                             channel, tags["display-name"], row["name"], cards, reward,
                                             first)).start()
            ...
            if subcmd in ["change", "request"]:
                ...
                with db.cursor(pymysql.cursors.DictCursor) as cur:
                    ...
                    if canManageImages:
                        ...
                        insertArgs = [tags['user-id'], setData["id"], args[2], tags['user-id'],
                                      current_milli_time()]
                        cur.execute(
                            "INSERT INTO setbadge_requests (requesterid, setid, image, state, moderatorid, created) VALUES(%s, %s, %s, 'auto_accepted', %s, %s)",
                            insertArgs)
                        ...
                        cur.execute(
                            "SELECT COALESCE(MAX(created), 0) AS lastReq FROM setbadge_requests WHERE state = 'accepted' AND setid = %s",
                            [setData["id"]])
                        lastRequest = cur.fetchone()["lastReq"]
                        cooldown = lastRequest + int(
                            config["imageChangeCooldownDays"]) * 86400000 - current_milli_time()

                        if cooldown > 0:
                            datestring = formatTimeDelta(cooldown, False)
                            self.message(channel,
                                         "Sorry, that set has had its badge changed too recently. Please try again in %s" % datestring,
                                         isWhisper)
                            return
                        ...
                        try:
                            ...
                            self.message(channel,
                                         "There was an unknown problem with the link you specified. Please try again later.",
                                         isWhisper)
                            return
                        # cancel any old pending requests for this set
                        cur.execute(
                            "UPDATE setbadge_requests SET state = 'cancelled', updated = %s WHERE setid = %s AND state = 'pending'",
                            [current_milli_time(), setData["id"]])

                        # record a new request
                        insertArgs = [tags['user-id'], setData["id"], args[2], current_milli_time()]
                        cur.execute(
                            "INSERT INTO setbadge_requests (requesterid, setid, image, state, created) VALUES(%s, %s, %s, 'pending', %s)",
                            insertArgs)

                        # notify the discordhook of the new request
                        discordArgs = {"user": tags['display-name'], "setid": setData["id"],
                                       "name": setData["name"], "image": args[2]}
                        discordbody = {
                            "username": "WTCG Admin",
                            "content": "{user} requested a set badge change for {name} to <{image}>!\nUse `!setbadge check {setid}` in any chat to check it.".format(
                                **discordArgs)
                        }
                        threading.Thread(target=sendAdminDiscordAlert, args=(discordbody,)).start()

                        self.message(channel,
                                     "Your request has been placed. You will be notified when bot staff accept or decline it.",
                                     isWhisper)
                        return
            elif subcmd == "list":
                with db.cursor() as cur:
                    cur.execute(
                        "SELECT sets.id, sets.name FROM setbadge_requests sr JOIN sets ON sr.setid=sets.id WHERE sr.requesterid = %s AND sr.state = 'pending'",
                        [tags['user-id']])
                    reqs = cur.fetchall()
                    ...
                    if len(reqs) == 0:
                        ...
                        self.message(channel, "%s, you have pending set badge change requests for: %s." % (
                            tags['display-name'], reqList), isWhisper)
                ...
                with db.cursor() as cur:
                    ...
                    if cur.rowcount > 0:
                        ...
                        discordbody = {
                            "username": "WTCG Admin",
                            "content": "{user} cancelled their badge change request for [{id}] {name}.".format(
                                **discordArgs)
                        }
                        threading.Thread(target=sendAdminDiscordAlert, args=(discordbody,)).start()
                        self.message(channel,
                                     "You cancelled your badge change request for [%d] %s." % (args[1], setName),
                                     isWhisper)
                    else:
                        self.message(channel, "You didn't have a pending badge change request for that set.",
                                     isWhisper)
                    return
                ...
                with db.cursor() as cur:
                    ...
                    if len(queue) == 0:
                        ...
                        self.message(channel,
                                     "Current requested IDs for set badge changes: %s. !setbadge check <id> to see each request." % queueStr,
                                     isWhisper)
                    return
                ...
                with db.cursor() as cur:
                    cur.execute(
                        "SELECT sr.id, sr.image, users.id, users.name, sets.id, sets.name, sets.badgeid FROM setbadge_requests sr"
                    ...
                    if subcmd == "check":
                        ...
                        self.message(channel,
                                     ("{user} requested {name}'s set badge image to be changed to {image} ." +
                                      " You can accept this request with !setbadge accept {setid}" +
                                      " or deny it with !setbadge reject {setid} <reason>.").format(**msgArgs),
                                     isWhisper)
                    elif subcmd == "reject":
                        if len(args) < 3:
                            self.message(channel,
                                         "You must provide a reason to reject the request. If it is porn/illegal/etc, just ban the user.",
                                         isWhisper)
                            return
                        rejectionReason = " ".join(args[2:])
                        queryArgs = [tags['user-id'], current_milli_time(), rejectionReason, request[0]]
                        cur.execute(
                            "UPDATE setbadge_requests SET state = 'rejected', moderatorid = %s, updated = %s, rejection_reason = %s WHERE id = %s",
                            queryArgs)

                        # notify them
                        self.message("#%s" % request[3],
                                     "Your badge change request for %s was rejected with the following reason: %s" % (
                                         request[5], rejectionReason), True)
                        ...
                        try:
                            ...
                            self.message(channel,
                                         "Could not process image. %s. Check the URL yourself and if it is invalid reject their request." % str(
                                             ex), isWhisper)
                            return
                        ...
                        cur.execute(
                            "UPDATE setbadge_requests SET state = 'accepted', moderatorid = %s, updated = %s WHERE id = %s",
                            queryArgs)

                        # notify them
                        self.message("#%s" % request[3],
                                     "Your set badge change request for %s was accepted." % request[5], True)

                        self.message(channel,
                                     "Request accepted. The new set badge for %s is %s" % (request[5], hostedURL),
                                     isWhisper)
                    return
        ...
        if command == "givefreepack" and sender in superadmins:
            if len(args) < 2:
                self.message(channel, "Usage: !givefreepack <username> <booster name> [<amount> (default 1)]",
                             isWhisper)
                return
            ...
            with db.cursor() as cur:
                ...
                if amount > 1:
                    self.message('#%s' % userData[1],
                                 "You were given %d free %s packs by an admin. Check them using !freepacks" % (
                                     amount, args[1]), True)
                else:
                    self.message('#%s' % userData[1],
                                 "You were given a free %s pack by an admin. Open it using !freepacks open %s" % (
                                     args[1], args[1]), True)

                self.message(channel, "Successfully gave %d %s packs to %s." % (amount, args[1], userData[1]),
                             isWhisper)
                return
        ...
        if command == "raritychange" and sender in superadmins:
            ...
            if not hasConfirmed and rarity > waifu['base_rarity'] and waifu['base_rarity'] < int(
                    config["numNormalRarities"]) - 1:
                # check for promoted copies existing
                with db.cursor() as cur:
                    cur.execute("SELECT COUNT(*) FROM cards WHERE waifuid = %s AND rarity BETWEEN %s AND %s",
                                [waifu['id'], waifu['base_rarity'] + 1, int(config["numNormalRarities"]) - 1])
                    if cur.fetchone()[0] > 0:
                        self.message(channel,
                                     "WARNING: You are trying to increase the rarity of a card which people have already promoted. This may cause undesirable results. Append ' yes' to your command if you want to do this anyway.",
                                     isWhisper)
                        return
            ...
            with db.cursor() as cur:
                if rarity < int(config['numNormalRarities']):
                    cur.execute(
                        "UPDATE waifus SET normal_weighting = LEAST(GREATEST(normal_weighting, (SELECT MIN(w1.normal_weighting) FROM (SELECT * FROM waifus) w1 WHERE w1.base_rarity = %s)), (SELECT MAX(w2.normal_weighting) FROM (SELECT * FROM waifus) w2 WHERE w2.base_rarity = %s)), base_rarity = %s WHERE id = %s",
                        [rarity, rarity, rarity, waifu['id']])
                else:
                    cur.execute("UPDATE waifus SET base_rarity = %s WHERE id = %s", [rarity, waifu['id']])

                cur.execute(
                    "SELECT id, userid, boosterid FROM cards WHERE userid IS NOT NULL AND waifuid = %s AND rarity < %s",
                    [waifu['id'], rarity])
                ...
                if rarity >= int(config["numNormalRarities"]):
                    cur.execute(
                        "UPDATE users SET favourite = 1 WHERE favourite = %s AND (SELECT COUNT(*) FROM cards WHERE cards.userid = users.id AND cards.boosterid IS NULL AND cards.waifuid = %s) = 0",
                        [waifu['id']] * 2)
        ...
        if command == "profile":
            if len(args) == 0:
                ...
                if not canLookup and not hasOrIsLowRarity:
                    ...
                    self.message(channel,
                                 "%s, sorry, but that Waifu can't be set as your favourite. Try a different one!" %
                                 tags['display-name'], isWhisper)
                    return
        ...
        if command == "godimage":
            ...
            if subcmd in ["change", "request"]:
                ...
                try:
                    ...
                    self.message(channel,
                                 "%s, you don't own that waifu/card or it isn't god rarity!" % tags['display-name'],
                                 isWhisper)
                    return
                except AmbiguousWaifuException:
                    self.message(channel,
                ...
                with db.cursor() as cur:
                    if card["base_rarity"] == godRarity:
                        self.message(channel, "Base god rarity waifus cannot have their picture changed!",
                                     isWhisper)
                        return
                    ...
                    if canManageImages:
                        ...
                        # log the change for posterity
                        insertArgs = [tags['user-id'], card['cardid'], args[2], tags['user-id'],
                                      current_milli_time()]
                        cur.execute(
                            "INSERT INTO godimage_requests (requesterid, cardid, image, state, moderatorid, created) VALUES(%s, %s, %s, 'auto_accepted', %s, %s)",
                            insertArgs)
                        ...
                        cur.execute(
                            "SELECT COALESCE(MAX(created), 0) FROM godimage_requests WHERE state = 'accepted' AND cardid = %s",
                            [card['cardid']])
                        ...
                        if cooldown > 0:
                            datestring = formatTimeDelta(cooldown, False)
                            self.message(channel,
                                         "Sorry, that card has had its image changed too recently. Please try again in %s" % datestring,
                                         isWhisper)
                            return
                        ...
                        cur.execute(
                            "UPDATE godimage_requests SET state = 'cancelled', updated = %s WHERE cardid = %s AND state = 'pending'",
                            [current_milli_time(), card['cardid']])

                        # record a new request
                        insertArgs = [tags['user-id'], card['cardid'], args[2], current_milli_time()]
                        cur.execute(
                            "INSERT INTO godimage_requests (requesterid, cardid, image, state, created) VALUES(%s, %s, %s, 'pending', %s)",
                            insertArgs)

                        # notify the discordhook of the new request
                        discordArgs = {"user": tags['display-name'], "waifuid": card['waifuid'],
                                       "cardid": card['cardid'], "name": card["name"], "image": args[2]}
                        discordbody = {
                            "username": "WTCG Admin",
                            "content": "{user} requested an image change for [{waifuid}] {name} to <{image}>!\nUse `!godimage check {cardid}` in any chat to check it.".format(
                                **discordArgs)
                        }
                        threading.Thread(target=sendAdminDiscordAlert, args=(discordbody,)).start()
                ...
                with db.cursor() as cur:
                    cur.execute(
                        "SELECT waifus.id, waifus.name FROM godimage_requests gr JOIN cards ON gr.cardid=cards.id JOIN waifus ON cards.waifuid = waifus.id WHERE gr.requesterid = %s AND gr.state = 'pending'",
                        [tags['user-id']])
                    reqs = cur.fetchall()
                    ...
                    if len(reqs) == 0:
                        ...
                        reqList = ", ".join(["[%d] %s" % (req[0], req[1]) for req in reqs])
                        self.message(channel, "%s, you have pending image change requests for: %s." % (
                            tags['display-name'], reqList), isWhisper)
                ...
                with db.cursor() as cur:
                    ...
                    if cur.rowcount > 0:
                        ...
                        discordbody = {
                            "username": "WTCG Admin",
                            "content": "{user} cancelled their image change request for [{id}] {name}.".format(
                                **discordArgs)
                        }
                        threading.Thread(target=sendAdminDiscordAlert, args=(discordbody,)).start()
                        self.message(channel, "You cancelled your image change request for [%d] %s." % (
                            card['waifuid'], card["name"]), isWhisper)
                    else:
                        self.message(channel, "You didn't have a pending image change request for that waifu.",
                                     isWhisper)
                    return
            elif subcmd == "queue" and canManageImages:
                with db.cursor() as cur:
                    ...
                    if len(queue) == 0:
                        ...
                        queueStr = ", ".join(str(item[0]) for item in queue)
                        self.message(channel,
                                     "Current requested IDs for image changes: %s. !godimage check <id> to see each request." % queueStr,
                                     isWhisper)
                    return
                ...
                with db.cursor() as cur:
                    cur.execute(
                        "SELECT gr.id, gr.image, users.id, users.name, waifus.id, waifus.name, cards.id FROM godimage_requests gr"
                    ...
                    if subcmd == "check":
                        msgArgs = {"user": request[3], "waifuid": request[4], "name": request[5],
                                   "image": request[1], "cardid": request[6]}
                        self.message(channel, (
                                "{user} requested their copy of [{waifuid}] {name}'s image to be changed to {image} ." +
                                " You can accept this request with !godimage accept {cardid}" +
                                " or deny it with !godimage reject {cardid} <reason>.").format(**msgArgs),
                                     isWhisper)
                    elif subcmd == "reject":
                        if len(args) < 3:
                            self.message(channel,
                        ...
                        queryArgs = [tags['user-id'], current_milli_time(), rejectionReason, request[0]]
                        cur.execute(
                            "UPDATE godimage_requests SET state = 'rejected', moderatorid = %s, updated = %s, rejection_reason = %s WHERE id = %s",
                            queryArgs)

                        # notify them
                        self.message("#%s" % request[3],
                                     "Your image change request for [%d] %s was rejected with the following reason: %s" % (
                                         request[4], request[5], rejectionReason), True)
                        ...
                        queryArgs = [tags['user-id'], current_milli_time(), request[0]]
                        cur.execute(
                            "UPDATE godimage_requests SET state = 'accepted', moderatorid = %s, updated = %s WHERE id = %s",
                            queryArgs)

                        # notify them
                        self.message("#%s" % request[3],
                                     "Your image change request for your copy of [%d] %s was accepted." % (
                                         request[4], request[5]), True)

                        self.message(channel, "Request accepted. The new image for %s's copy of [%d] %s is %s" % (
                            request[3], request[4], request[5], hostedURL), isWhisper)
                    return
        if command == "sendpoints":
            # expire old points transfers
            with db.cursor(pymysql.cursors.DictCursor) as cur:
                cur.execute(
                    "UPDATE points_transfers SET status = 'expired' WHERE status = 'pending' AND created <= %s",
                    [current_milli_time() - int(config["pointsTransferExpiryMinutes"]) * 60000])
                if len(args) < 2:
                    self.message(channel,
                                 "Usage: !sendpoints <user> <amount> <reason> OR !sendpoints confirm <code>",
                                 isWhisper)
                    return
                ...
                if args[0].lower() == "confirm":
                    ...
                    cur.execute(
                        "SELECT pt.*, users.name AS toName FROM points_transfers AS pt JOIN users ON pt.toid = users.id WHERE pt.fromid = %s ORDER BY pt.id DESC LIMIT 1",
                        [tags['user-id']])
                    transfer = cur.fetchone()

                    if transfer is None or transfer["status"] in ["confirmed", "confirm_failed"]:
                        self.message(channel, "%s, you have no pending points transfer." % tags['display-name'],
                                     isWhisper)
                        return

                    if transfer["status"] == "expired":
                        self.message(channel,
                                     "%s, your points transfer has expired. Please try sending a new one." % tags[
                                         'display-name'], isWhisper)
                        return

                    if "%s-%d" % (transfer["toName"], transfer["paid"]) != code:
                        cur.execute("UPDATE points_transfers SET status='confirm_failed' WHERE id = %s",
                                    [transfer["id"]])
                        self.message(channel,
                                     "%s, the confirmation code you entered was wrong. Please try sending a new points transfer." %
                                     tags['display-name'], isWhisper)
                        return
                    ...
                    if not hasPoints(tags['user-id'], transfer["paid"]):
                        cur.execute("UPDATE points_transfers SET status='confirm_failed' WHERE id = %s",
                                    [transfer["id"]])
                        self.message(channel,
                                     "%s, you no longer have enough points to complete this transfer. Please try sending a new points transfer." %
                                     tags['display-name'], isWhisper)
                        return
                    ...
                    if int(transfer["sent"]) >= int(config["pointsTransferMinWhisperAmount"]):
                        self.message("#%s" % transfer["toName"], "%s sent you %d points with the reason: %s" % (
                            tags["display-name"], transfer["sent"], transfer["reason"]), True)

                    cur.execute("UPDATE points_transfers SET status='confirmed', confirmed=%s WHERE id = %s",
                                [current_milli_time(), transfer["id"]])

                    self.message(channel, "%s, you successfully paid %d points to send %d points to %s." % (
                        tags["display-name"], transfer["paid"], transfer["sent"], transfer["toName"]))
                else:
                    if len(args) < 3:
                    ...
                    if not hasPoints(tags['user-id'], toPay):
                        self.message(channel, "You don't have enough points to send %d points. You need %d." % (
                            amount, toPay), isWhisper)
                        return

                    insertArgs = [tags['user-id'], otherid, amount, toPay, current_milli_time(), reason]
                    cur.execute(
                        "INSERT INTO points_transfers (fromid, toid, sent, paid, status, created, reason) VALUES(%s, %s, %s, %s, 'pending', %s, %s)",
                        insertArgs)

                    msgArgs = (tags["display-name"], amount, otherparty, toPay, otherparty, toPay)
                    self.message(channel,
                                 "%s, you want to send %d points to %s. This will cost you %d points. To confirm this action, enter !sendpoints confirm %s-%d" % msgArgs,
                                 isWhisper)
                return
        if command == "sorthand":
            if len(args) == 0 or (args[0].lower() != "reset" and len(args) < 2):
                self.message(channel,
                             "Usage: !sorthand <comma-delimited card ids> <comma-delimited positions> OR !sorthand reset",
                             isWhisper)
                return
            with db.cursor() as cur:
                if args[0].lower() == "reset":
                    ...
                    self.message(channel,
                                 "Successfully reset %s's hand to default sort order." % tags['display-name'],
                                 isWhisper)
                else:
                    ...
                    for i in range(len(cardSpecifiers)):
                        try:
                            ...
                            if sortValue < -32768 or sortValue > 32767:
                                self.message(channel,
                                             "Invalid sort value entered. Valid values are -32768 to 32767 inclusive.",
                                             isWhisper)
                                return
                    ...
                    self.message(channel, "Updated sort values for %d cards in %s's hand." % (
                        len(updatePairs), tags['display-name']), isWhisper)
                return
        if command == "autogacha" and sender in superadmins:
            tokenName = config["eventTokenName"]
            with db.cursor() as cur:
                cur.execute(
                    "SELECT id, name, eventTokens FROM users WHERE eventTokens > 0 ORDER BY eventTokens DESC")
                ...
                for holder in holders:
                    ...
                    for i in range(int(holder[2])):
                        ...
                        if "pack" in roll["prize"]:
                            ...
                            prizes.append("%dx %s pack (!freepacks open %s)" % (
                                roll["prize"]["amount"], roll["prize"]["pack"], roll["prize"]["pack"]))
                    ...
                    messages = ["Your %d leftover %s(s) were fed into the Token Gacha and you got: " % (
                        holder[2], tokenName)]
                    first = True
        ...
        if command == "tokenshop":
            ...
            with db.cursor() as cur:
                cur.execute(
                    "SELECT annivPromosBought, annivSpecialBought, annivHandUpgradeBought, eventTokens FROM users WHERE id = %s",
                    [tags['user-id']])
                ...
                if booleanConfig("annivShopSpecialOnly") and subcmd not in ["special", "specialadmin"]:
                    self.message(channel,
                                 "Token Shop purchases are closed for this year, thanks for playing! If you've already purchased a Special and don't have it in your hand yet, you still have access to !tokenshop special commands.",
                                 isWhisper)
                    return
                if subcmd == "gacha":
                    tokenName = config["eventTokenName"]
                    if len(args) == 1 or args[1].lower() != "roll":
                        self.message(channel,
                                     "!tokenshop gacha roll to try your luck on the %s Gacha. 1 %s per go." % (
                                         tokenName, tokenName), isWhisper)
                        return
                    ...
                    if tokens < 1:
                        self.message(channel, "You don't have any %ss to roll the Gacha with." % (tokenName),
                                     isWhisper)
                        return
                    ...
                    self.message(channel, "%s, you roll the %s Gacha and you get: [%d◆] %s" % (
                        tags['display-name'], tokenName, roll["tier"], prizeStr), isWhisper)
                    return
                if subcmd == "buy":
                    if len(args) < 2:
                        self.message(channel,
                                     "Usage: !tokenshop buy special | !tokenshop buy handupgrade | !tokenshop buy <promo waifu id>",
                                     isWhisper)
                        return
                    ...
                    if target == "special":
                        if purchaseData[1]:
                            self.message(channel,
                                         "%s, you've already bought a special card! Pick another reward." % tags[
                                             'display-name'], isWhisper)
                            return
                        ...
                        if purchaseData[3] < specialCost:
                            ...
                            self.message(channel,
                                         "%s, you can't afford a special card! They cost %d tokens and you only have %d." % msgArgs,
                                         isWhisper)
                            return
                        ...
                        cur.execute(
                            "UPDATE users SET eventTokens = eventTokens - %s, annivSpecialBought = 1 WHERE id = %s",
                            [specialCost, tags['user-id']])
                        cur.execute(
                            "INSERT INTO special_requests (requesterid, state, created) VALUES(%s, 'notsent', %s)",
                            [tags['user-id'], current_milli_time()])
                        self.message(channel,
                                     "%s, you bought a special card for %d tokens! You now have access to !tokenshop special which you can use to set the name/series/image of your special before submitting it for review." % (
                                         tags['display-name'], specialCost), isWhisper)
                        ...
                        if purchaseData[2]:
                            self.message(channel,
                                         "%s, you've already bought a hand upgrade! Pick another reward." % tags[
                                             'display-name'], isWhisper)
                            return
                        ...
                        if purchaseData[3] < huCost:
                            ...
                            self.message(channel,
                                         "%s, you can't afford a hand upgrade! They cost %d tokens and you only have %d." % msgArgs,
                                         isWhisper)
                            return

                        # ok, make the purchase
                        cur.execute(
                            "UPDATE users SET eventTokens = eventTokens - %s, annivHandUpgradeBought = 1, freeUpgrades = freeUpgrades + 1 WHERE id = %s",
                            [huCost, tags['user-id']])
                        self.message(channel,
                                     "%s, you bought a hand upgrade for %d tokens! You can begin using it immediately." % (
                                         tags['display-name'], huCost), isWhisper)
                    else:
                        # promo
                        if purchaseData[0] >= maxPromos:
                            self.message(channel,
                                         "%s, you have already bought the maximum of %d promos. Pick another reward." % (
                                             tags['display-name'], maxPromos), isWhisper)
                            return
                        ...
                        if purchaseData[3] < promoPrice:
                            ...
                            self.message(channel,
                                         "%s, you can't afford a promo! Your current cost for one is %d tokens and you only have %d." % msgArgs,
                                         isWhisper)
                            return
                        ...
                        if promoCard is None:
                            self.message(channel, "%s, you entered an invalid waifu ID." % tags['display-name'],
                                         isWhisper)
                            return

                        if not promoCard["can_purchase"]:
                            self.message(channel,
                                         "%s, that waifu cannot be bought. !tokenshop listpromos to get a list of buyable promos." %
                                         tags['display-name'], isWhisper)
                            return

                        addCard(tags['user-id'], promoCard['id'], 'other', None,
                                int(config["numNormalRarities"]) + int(config["numSpecialRarities"]) - 1)
                        cur.execute(
                            "UPDATE users SET eventTokens = eventTokens - %s, annivPromosBought = annivPromosBought + 1 WHERE id = %s",
                            [promoPrice, tags['user-id']])
                        if purchaseData[0] + 1 < maxPromos:
                            msgArgs = (tags['display-name'], promoCard['id'], promoCard['name'], promoPrice,
                                       purchaseData[0] + 1, promoPrice + promoCost)
                            self.message(channel,
                                         "%s, you bought a copy of [%d] %s for %d tokens. You have now bought %d promos so your next one will cost %d tokens." % msgArgs,
                                         isWhisper)
                        else:
                            msgArgs = (tags['display-name'], promoCard['id'], promoCard['name'], promoPrice,
                                       purchaseData[0] + 1)
                            self.message(channel,
                                         "%s, you bought a copy of [%d] %s for %d tokens. You have now bought %d promos so you cannot buy any more." % msgArgs,
                                         isWhisper)
                elif subcmd == "listpromos":
                    self.message(channel, "%s, you can view the list of promos buyable with tokens here - %s" % (
                        tags['display-name'], config["tokenPromoList"]), isWhisper)
                    ...
                    if spclcmd in ["check", "accept", "reject"]:
                        if len(args) < 3:
                            ...
                            return
                        cur.execute(
                            "SELECT sr.requesterid, sr.name, sr.series, sr.image FROM special_requests sr JOIN users ON sr.requesterid=users.id WHERE users.name = %s AND sr.state = 'pending'",
                            [args[2]])
                        requestInfo = cur.fetchone()

                        if requestInfo is None:
                            self.message(channel, "That user doesn't have a pending Special card request.",
                                         isWhisper)
                            return
                        ...
                        if spclcmd == "check":
                            ...
                            self.message(channel,
                                         "%s requested a Special card of %s from %s with image %s . Accept it with !tokenshop specialadmin accept %s or reject it with !tokenshop specialadmin reject %s <reason>." % msgArgs,
                                         isWhisper)
                        elif spclcmd == "accept":
                            # create waifu
                            try:
                                ...
                                self.message(channel,
                                             "Could not process image. %s. Check the URL yourself and if it is invalid reject their request." % str(
                                                 ex), isWhisper)
                                return
                            queryArgs = [requestInfo[1], hostedURL, int(config["numNormalRarities"]),
                                         requestInfo[2]]
                            cur.execute(
                                "INSERT INTO waifus (name, image, base_rarity, series) VALUES(%s, %s, %s, %s)",
                                queryArgs)
                            waifuid = cur.lastrowid

                            addCard(requestInfo[0], waifuid, 'other')
                            cur.execute(
                                "UPDATE special_requests SET state = 'accepted', moderatorid = %s, updated = %s WHERE requesterid = %s",
                                [tags['user-id'], current_milli_time(), requestInfo[0]])
                            self.message(channel,
                                         "Request accepted. Created Special card ID %d and gave it to %s." % (
                                             waifuid, args[2]), isWhisper)
                            self.message('#%s' % args[2].lower(),
                                         "Your Special card request for %s from %s was accepted." % (
                                             requestInfo[1], requestInfo[2]), True)
                        else:
                            # reject
                            if len(args) < 4:
                            ...
                            cur.execute(
                                "UPDATE special_requests SET state = 'rejected', moderatorid = %s, updated = %s, rejection_reason = %s, changed_since_rejection = 0 WHERE requesterid = %s",
                                [tags['user-id'], current_milli_time(), rejectionReason, requestInfo[0]])
                            self.message('#%s' % args[2].lower(),
                                         "Your Special card request for %s from %s was rejected for the following reason: %s. Please adjust it with !tokenshop special and then resubmit." % (
                                             requestInfo[1], requestInfo[2], rejectionReason), True)
                            self.message(channel, "Request rejected and user notified.", isWhisper)
                    else:
                        # list pending
                        cur.execute(
                            "SELECT users.name FROM special_requests sr JOIN users ON sr.requesterid=users.id WHERE sr.state='pending' ORDER BY sr.updated ASC")
                        results = cur.fetchall()

                        if not results:
                            self.message(channel, "There are no pending Special card requests right now.",
                                         isWhisper)
                        else:
                            self.message(channel,
                                         "Special card requests have been received from: %s. Use !tokenshop specialadmin check <username> for details." % (
                                             ", ".join([row[0] for row in results])), isWhisper)
                elif subcmd == "special":
                    if not purchaseData[1]:
                        self.message(channel, "%s, you haven't bought a Special card with tokens yet!" % tags[
                            'display-name'], isWhisper)
                        return
                    ...
                    cur.execute(
                        "SELECT name, series, image, state, updated, rejection_reason, changed_since_rejection FROM special_requests WHERE requesterid = %s",
                        [tags['user-id']])
                    ...
                    if spclcmd in ["name", "series", "image", "submit", "confirm"] and specialData[3] not in [
                        "notsent", "rejected"]:
                        if specialData[3] == "pending":
                            self.message(channel,
                                         "%s, you have already sent in your Special card request! Please wait for a response." %
                                         tags['display-name'], isWhisper)
                        else:
                            self.message(channel, "%s, your Special card request has already been accepted!" % tags[
                                'display-name'], isWhisper)
                        return

                    if spclcmd in ["submit", "confirm"] and specialData[3] == "rejected" and not specialData[6]:
                        self.message(channel,
                                     "%s, you haven't changed your request at all since its rejection! Please review the rejection reason (%s) and make adjustments before resubmitting." % (
                                         tags['display-name'], specialData[5]), isWhisper)
                        return

                    if spclcmd in ["submit", "confirm"] and (
                            specialData[0] is None or specialData[1] is None or specialData[2] is None):
                        self.message(channel,
                                     "%s, you haven't finished filling in the info for your Special card request yet! Please specify all 3 of name/series/image before continuing." %
                                     tags['display-name'], isWhisper)
                        return

                    if spclcmd in ["name", "series", "image"] and len(args) < 3:
                        self.message(channel,
                                     "Usage: !tokenshop special name <name> / series <series> / image <image URL> / submit")
                        return

                    if spclcmd == "name":
                        name = (" ".join(args[2:])).strip()
                        if not len(name) or len(name) > 50:
                            self.message(channel, "Please enter a waifu name of between 1 and 50 characters.",
                                         isWhisper)
                            return

                        queryArgs = [name, current_milli_time(), tags['user-id']]
                        cur.execute(
                            "UPDATE special_requests SET name = %s, changed_since_rejection = 1, updated = %s WHERE requesterid = %s",
                            queryArgs)
                        self.message(channel, "%s -> Set the name for your Special card request to %s." % (
                            tags['display-name'], name), isWhisper)
                    elif spclcmd == "series":
                        series = (" ".join(args[2:])).strip()
                        if not len(series) or len(series) > 50:
                            self.message(channel, "Please enter a waifu series of between 1 and 50 characters.",
                                         isWhisper)
                            return

                        queryArgs = [series, current_milli_time(), tags['user-id']]
                        cur.execute(
                            "UPDATE special_requests SET series = %s, changed_since_rejection = 1, updated = %s WHERE requesterid = %s",
                            queryArgs)
                        self.message(channel, "%s -> Set the series for your Special card request to %s." % (
                            tags['display-name'], series), isWhisper)
                    elif spclcmd == "image":
                        try:
                            validateWaifuURL(args[2])
                        except ValueError as ex:
                            ...
                            self.message(channel,
                                         "There was an unknown problem with the image link you specified. Please try again later.",
                                         isWhisper)
                            return

                        queryArgs = [args[2], current_milli_time(), tags['user-id']]
                        cur.execute(
                            "UPDATE special_requests SET image = %s, changed_since_rejection = 1, updated = %s WHERE requesterid = %s",
                            queryArgs)
                        self.message(channel, "%s -> Set the image for your Special card request to %s" % (
                            tags['display-name'], args[2]), isWhisper)
                    elif spclcmd == "submit":
                        self.message(channel,
                                     "%s, please confirm that you want a Special card of %s from %s with image %s by entering !tokenshop special confirm. You cannot change your submission after it is confirmed so please be careful." % (
                                         tags['display-name'], specialData[0], specialData[1], specialData[2]),
                                     isWhisper)
                        ...
                        threading.Thread(target=sendAdminDiscordAlert, args=(discordbody,)).start()
                        cur.execute("UPDATE special_requests SET state = 'pending' WHERE requesterid = %s",
                                    [tags['user-id']])
                        self.message(channel,
                                     "%s, your Special card request has been submitted for approval. Keep an eye on your whispers to see if it is accepted or rejected. If it is rejected you'll be able to resubmit after you fix the problems." %
                                     tags['display-name'], isWhisper)
                    else:
                        # show status
                        if specialData[3] == "accepted":
                            self.message(channel, "%s, your Special card request of %s from %s was accepted." % (
                                tags['display-name'], specialData[0], specialData[1]), isWhisper)
                        elif specialData[3] == "rejected":
                            if specialData[6]:
                                self.message(channel,
                                             "%s, your previous submission was rejected for the following reason: %s. Your current submission is %s from %s with image %s . !tokenshop special name/image/series to change details or !tokenshop special submit to submit it for approval." % (
                                                 tags['display-name'], specialData[5], specialData[0],
                                                 specialData[1],
                                                 specialData[2]), isWhisper)
                            else:
                                self.message(channel,
                                             "%s, your submission of %s from %s with image %s was rejected for the following reason: %s. Please use !tokenshop special name/image/series to change details to solve the problems and then !tokenshop special submit to resubmit it for approval." % (
                                                 tags['display-name'], specialData[0], specialData[1],
                                                 specialData[2],
                                                 specialData[5]), isWhisper)
                        elif specialData[3] == "pending":
                            self.message(channel,
                                         "%s, your Special card request of %s from %s is still pending approval. Please wait patiently." % (
                                             tags['display-name'], specialData[0], specialData[1]), isWhisper)
                        else:
                            ...
                            if specialData[0] and specialData[1] and specialData[2]:
                                self.message(channel,
                                             "%s, your current Special request is %s from %s with image %s . Use !tokenshop special name/image/series to change details or !tokenshop special submit to submit it for approval." % (
                                                 tags['display-name'], name, series, image), isWhisper)
                            elif specialData[0] or specialData[1] or specialData[2]:
                                self.message(channel,
                                             "%s, your current Special request is %s from %s with image %s . Use !tokenshop special name/image/series to add the missing details and then !tokenshop special submit to submit it for approval." % (
                                                 tags['display-name'], name, series, image), isWhisper)
                            else:
                                self.message(channel,
                                             "%s, you haven't started on your Special request yet. Use !tokenshop special name/image/series to add the required details and then !tokenshop special submit to submit it for approval." %
                                             tags['display-name'], isWhisper)
                else:
                    # list items
                    purchasable = []
                    ...
                    if purchaseData[0] < maxPromos:
                        promoPrice = promoCost * (purchaseData[0] + 1)
                        purchasable.append(
                            "Promo card - current cost %d tokens (%d already bought, %d max) - !tokenshop buy <waifu id> (!tokenshop listpromos for a list)" % (
                                promoPrice, purchaseData[0], maxPromos))
                    ...
                    self.message(channel,
                                 "%s, you have %d Anniversary Tokens. Items currently available to you: %s" % (
                                     tags['display-name'], purchaseData[3], " / ".join(purchasable)), isWhisper)
            return
...
class MarathonFFZWebsocket:
    def __init__(self, channelName, newFollowButtons=None):
        ...
        self.ws = websocket.WebSocketApp(ffzws, on_message=self.on_message, on_error=self.on_error,
